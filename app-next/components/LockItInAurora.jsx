...
export default function LockItInAurora({ entry, accent = '#3DE88A', onComplete, onSwitch }) {
  ...
  // ---- Payoff canvas: node-bloom, connectedness scaled by score ----
  useEffect(() => {
    if (!payoff || !result || !canvasRef.current) return
    ...
    const cx = W / 2, cy = H / 2
    const N = 7
    const connected = result.score >= 3 ? 7 : result.score === 2 ? 5 : 3
    const nodes = []
    for (let i = 0; i < N; i++) {
    ...
    const speed = result.score >= 3 ? 0.0026 : 0.0018
    let f = 0
    const draw = () => {
