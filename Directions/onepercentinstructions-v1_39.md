...
**Version 1.39 | Updated June 28, 2026**
...
**lockins** (NEW v1.39 — spaced repetition): `id`, `user_id`→profiles, `entry_number`, `concept`, `category`, `keeper`, `hook`, `box` (smallint 0–3), `status` (active/done/archived), `due_at`, `last_reviewed_at`, `review_count`, `reminder_sent_at`, `created_at`. Unique (user_id, entry_number). Explicit per-operation RLS (own rows only).

**Edge functions + pg_cron:** `send-daily-reminder` (0 12 * * *), `send-practice-reminder` (0 * * * *), `send-weekly-wrap` (0 11 * * *), **`send-lockin-review` (0 14 * * *, NEW v1.39)**. Cron-called functions must have **Verify JWT OFF** (tokenless call). Source in `supabase/functions/`.
...
CLAUDE_API_KEY=[set in Vercel — powers Deep Cut /api/deep-cut + Lock It In /api/lock-it-in]; ANTHROPIC_API_KEY honored as fallback
```
