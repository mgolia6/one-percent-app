...
**Version 1.42 | Updated July 1, 2026 (Session 5)**
...
| v1.35 | 2026-06-01 | Wake words updated: Platform Upgrades → Enhance, Repo Check → Repo, Let's Wrap It Up → Wrap. All wake words now require standard session review (clone/pull → directions → log → state snapshot) before execution. |
| v1.36 | 2026-06-12 | Badge system, dark mode overhaul, email system, PostHog analytics, goal-setting. |
| v1.37 | 2026-06-15 | UI-001–007 shipped. Deep Cut AI agent (circle FAB, color cycling, entry picker, streaming chat, source grounding). Profile overhaul (phone, email_reminders, About/Privacy links). Privacy policy page. Welcome back typewriter. Streak month grid. ABOUT removed from action strip. DB: profiles.phone, profiles.email_reminders, ANTHROPIC_API_KEY added to Vercel. |
| v1.38 | 2026-06-15 | Wrap protocol overhauled — exhaustive checklist covering every doc, every file, every system. No more missed updates at session close. |
| v1.39 | 2026-06-28 | **Lock It In** (AI-coached recall, `/api/lock-it-in`) live for ALL users + keeper quality gate. **Keep It Sharp** spaced repetition: `lockins` table, `/review` page, `lib/lockins.js` (Leitner 2d/5d/12d/30d), `send-lockin-review` edge fn + daily cron. **Categories 7→10** (added History/Personal Finance/Health & Performance) via single-source registry `lib/categories.js`. Env var corrected to **`CLAUDE_API_KEY`**. Domain **onepercent.mpgink.com**. Changelog v1.0 announcement modal. |
| v1.41 | 2026-06-28 (S3) | **"Why Today" rule locked:** `morning.why_today` = present-day relevance, NOT citations (those go in `sources`). Fixed ~28 live entries that were citation dumps. 60 lessons drafted (30 new-category + 30 rotation) in `Drafts/`, pending Dead Drop. |
| v1.40 | 2026-06-28 (S2) | **On This Day** daily bonus: `on_this_day` table, `/on-this-day` archive, `/api/on-this-day` (Wikipedia + Claude, self-caches via service role, `force-dynamic`/`no-store`), cron `generate-on-this-day` (jobid 5), admin backfill. **Admin overhaul:** dark theme, feedback summary + `/api/admin/feedback-summary`, check-in surveys + KV-table parse, addressed toggle, admin phone entry, systems strip, bug triage, **fixed API-health email-blast bug** + safe `/api/health`, **analytics PostHog proxy** `/api/admin/analytics`. New RLS: `feedback_update_admin`, `profiles_update_admin`, `is_admin()`. New env: **`SUPABASE_SERVICE_ROLE_KEY`** (set), **`POSTHOG_PERSONAL_KEY`** (needed). |
| v1.42 | 2026-06-30 → 07-01 (S4–S5) | **New locked patterns: Push Protocol** (ask push-to-main? + admin-gate? default gate=yes; exception for shared UI) and **Context Checkpoint Protocol** (`PreCompact` hook auto-commits in-flight work to the feature branch before the ~1M-token window compacts — `.claude/scripts/context-checkpoint.sh`). **S4:** /verify built into a real verification workstation (per-claim flags, Submit-for-Review batches, ⌗ Runs archive, lifecycle chips, source tiers/📍 locate/⧉ copy-search) + 4 admin-only tables (`verification_checks/_entries/_submissions/_category_state`); 30 rotation drafts verified; Finance/Health sources upgraded; editionId renumber (flat per category); **Today/On-Deck → oldest uncompleted unlocked**; integrated top action strip; Library Lessons/Prompts sub-tabs + Review in bottom nav (admin-gated); natural-language "why I'm here"; `scripts/promote.mjs` interleave-at-end mode; design docs (DESIGN-SYSTEM-BRIEF / DESIGN-MOMENTS / WORKING-WITH-CLAUDE-DESIGN). **S5:** branded **favicon + installable PWA** (`app/icon.svg` neon "1%", `app/favicon.ico`, `app/apple-icon.png`, `app/manifest.js`, maskable/192/512 icons, `viewport.themeColor` `#0e141c`). |
...
## Push Protocol (MANDATORY before every push) — locked S4

Before pushing any change, ask Matthew two questions and wait:
1. **Push to `main`?** — `main` auto-deploys to production via Vercel. Alternative is holding on the feature branch / preview.
2. **Admin-gate it?** — **DEFAULT IS YES.**

Best default for new work: ship to `main` but gated behind `is_admin`, so Matthew verifies it live without exposing it to all users. **Exception — shared UI edits** (existing UI everyone already sees, e.g. home header/favicon): not gateable — say so and just confirm the push-to-`main` decision. Full text in CLAUDE.md.

---

## Context Checkpoint Protocol — locked S5

The conversation window (~1M tokens) **auto-compacts** when full (summarizes + continues; no hard stop). Compaction is lossy and the container is ephemeral, so protect un-committed work:
1. **Automatic** — a `PreCompact` hook (`.claude/scripts/context-checkpoint.sh`, wired in `.claude/settings.json`) fires just before any compaction and commits uncommitted work to the **feature branch** (never `main`) as a `WIP context checkpoint`, then pushes. Safe to squash later.
2. **Soft (Claude's job)** — when the window runs long, proactively flush open decisions into `State/onepercentstate.md` + a one-liner in the current log, and tell Matthew a checkpoint ran.

Rule: if it's committed+pushed, in a repo file, or in Supabase, compaction can't hurt it. Full text in CLAUDE.md.

---

## Wake Words

All wake words require standard session review before execution:
**Clone/pull → Read latest Directions file → Read latest log → Read state snapshot → Then execute.**

| Phrase | Action |
|---|---|
| **One Percent** | Standard review, then check backlog and generate next entry |
| **Backlog** | Standard review, then work on concept pipeline |
| **Verify** | Standard review, then step-by-step source validation |
| **Repo** | Standard review, then status audit |
| **Wrap** | Standard review, then session cleanup — changelog, state snapshot, logs, push |
| **Bugs** | Standard review, then review bugs from Supabase |
| **Feedback** | Standard review, then review user feedback from Supabase |
| **Enhance** | Standard review, then work on app improvements (see product backlog) |
...
## Wrap Protocol — EXHAUSTIVE CHECKLIST

When Matthew says **"Wrap"**, Claude executes every step below without being asked. This is not optional. Missing any step is a failure. Matthew should not have to remind Claude to do any of this.

---

### Step 1 — Session log
- Write `Logs/onepercentlog[YYYY-MM-DD].md`
- If a log for today already exists, append as `## Session 2` (never create `-v2` files)
- Include: what was built, what was changed, what was decided, what's still open, next priorities

---

### Step 2 — Supabase changelog entry
- Draft plain-English changelog for everything visible that shipped this session
- Insert into Supabase `changelog` table with `published = false`
- Tell Matthew the draft — he approves or edits before it goes live
- Format: bullet points, plain English, no jargon, no commit hashes
...
**Version numbering:** App visible state, not instructions version. Increment by 0.1 per session that ships visible changes.

---

### Step 3 — State snapshot (`State/onepercentstate.md`)
Regenerate completely. Fields to update:
- Last entry generated (number, ID, concept)
- Next entry (number, category, rotation slot, candidates)
- Top priorities for next session (honest, reordered based on what happened)
- Active issues (add new, remove resolved)
- Platform upgrades queued (add new, remove completed)
- Approved builds not yet built
- All major features/systems currently live
...
- Mark completed items as ~~done~~ with ✅ date and one-line summary
- Add any new issues discovered this session
- Add any new enhancement ideas that came up
- Update schema notes if DB columns were added/changed
- Update notification system notes if email/push infra changed

---

### Step 5 — LAYOUT.md
Every UI change ships with a LAYOUT.md update. Check every section:
- **Design system** — any new tokens, colors, patterns?
- **Mobile/desktop layout** — any structural changes to header, nav, action strip?
- **Surfaces** — any tab content changed? New surfaces added?
- **Deep Cut** — if changed, update the Deep Cut section
- **Modals & Overlays** — any modal restyled or added?
- **Routes table** — any new routes? Any routes changed?
- **Key Layout Decisions** — anything locked or unlocked?
- **Open Layout Debt** — cross off resolved items, add new ones
- **Files table** — any new files that own a surface?

If nothing in the layout changed this session, write "No layout changes this session" and skip. But check first.

---

### Step 6 — Instructions file (`Directions/`)
Create a new version (`onepercentinstructions-v1_XX.md`) if ANY of the following changed:
- Wake words or their behavior
- Wrap protocol itself
- Design system tokens
- Supabase schema
- File structure / repo conventions
- App routes
- New env vars
- New tools or third-party integrations
- Any locked patterns or non-negotiables

Rules:
- Never edit a previous version — always create a new numbered file
- Add the version to the changelog table at the top of the file
- Previous versions stay in `Directions/` root until count exceeds 5, then move oldest to `Directions/Archive/`

---

### Step 7 — Content backlog (`Backlog/onepercentbacklog.md`)
Only if content work happened this session:
- Mark consumed concepts as used
- Add any new concepts that came up in discussion
- Update rotation notes if the next entry position changed

---

### Step 8 — Final build check
Run `cd app-next && npm run build` one last time before the final push.
If it fails — fix it before pushing. Never push a broken build.

---

### Step 9 — Commit and push everything
Single commit with all doc updates + any final code changes:
```bash
git add -A
git commit -m "Session wrap [YYYY-MM-DD] — [one-line summary of session]"
git push
```
...
Tell Matthew:
- What was shipped (one sentence per item)
- What docs were updated (checklist confirmation)
- What's first up next session
- Changelog version drafted (published = false, awaiting approval)

---

### What "Wrap" is NOT
- Wrap is not just pushing code
- Wrap is not just writing the log
- Wrap is not done until every doc in this checklist has been reviewed and updated
- If Matthew has to ask "did you update X?" — the wrap failed

---

### Changelog insert format reference
```sql
INSERT INTO changelog (version, title, description, published)
VALUES ('[x.x]', '[Plain English title]', E'• Change one\n• Change two\n• Change three', false);
```
...
| CM | Communication |
| HS | History (added v1.39) |
| PF | Personal Finance (added v1.39) |
| HP | Health & Performance (added v1.39) |

> Category identity (name, code/prefix, accent color, scope) is now defined ONCE in `app-next/lib/categories.js` (10 categories). Import from there — do not re-declare. The 3 new categories have no live entries yet and stay hidden from user-facing lists until they do.
...
└── app-next/
    ├── app/
    │   ├── page.js                — library/home (Deep Cut FAB mounted here)
    │   ├── entry/[id]/page.js
    │   ├── admin/page.js
    │   ├── about/page.js
    │   ├── changelog/page.js
    │   ├── privacy/page.js        — Privacy policy (added Jun 15)
    │   ├── login/page.js
    │   ├── onboarding/page.js
    │   ├── profile/page.js        — Account + Badges tabs only (Progress tab removed Jun 15)
    │   └── api/deep-cut/route.js  — Deep Cut streaming API (added Jun 15)
    ├── components/EntryViewer.jsx
    ├── components/DeepCut.jsx     — DeepCut drawer + DeepCutFAB export (added Jun 15)
    ├── lib/supabase.js
    ├── lib/unlock.js
...
| History | HS | #E0A93D |
| Personal Finance | PF | #3DE88A |
| Health & Performance | HP | #FF5151 |

> Rotation is `SC → AI → VL → SC → MM → AI → PH → NC → CM` for the original 7; **needs re-balancing for 10 categories** once the new three have entries (see State).
...
14. **Tester feedback rules (MANDATORY):**
    ...
    - **"Why Today" rule (MANDATORY, v1.41):** `morning.why_today` must argue PRESENT-DAY RELEVANCE — why this concept matters *right now* (the modern context: technology, workplace, culture, the moment). It is NOT a citation slot. Author names, works, and years belong in the `sources` array, never in why_today. If it reads like a bibliography, it's wrong. (Caught Session 3: ~28 live entries had citation-dump why_today; all fixed.)
...
**Standard method (Claude in Chrome — preferred):**
- Connect Claude in Chrome extension in Matthew's browser before starting
- Claude navigates directly to each source URL, reads the page content, and reports findings
- Matthew reviews Claude's summary and gives explicit human sign-off per claim
- This is faster than manual search but the human sign-off requirement does not change — Matthew must confirm each claim after Claude reports
...
**Both methods — shared rules:**
- Wait for Matthew's explicit confirmation before moving to next claim
...
- Claude in Chrome finding a URL does not constitute verification — Matthew's sign-off is always required
...
- **Hosting:** Vercel — **`https://onepercent.mpgink.com`** (primary, v1.39); `one-percent-app.vercel.app` still serves as alias
...
**profiles:** `id`, `email`, `name`, `first_name`, `last_name`, `signup_date`, `current_streak`, `longest_streak`, `last_active_date`, `onboarding_complete`, `is_admin`, `avatar_url`, `last_seen_changelog_version`, `phone` (text, optional, added Jun 15), `email_reminders` (boolean default true, added Jun 15), `goal_what`, `goal_when`, `goal_signal`, `streak_freezes`

**completions:** `user_id`, `entry_number`, `score`, `time_to_quiz`, `answers` (jsonb), `completed_at` (queried Jun 15 — powers streak month grid)
...
**lockins** (v1.39 — spaced repetition): `id`, `user_id`→profiles, `entry_number`, `concept`, `category`, `keeper`, `hook`, `box` (smallint 0–3), `status` (active/done/archived), `due_at`, `last_reviewed_at`, `review_count`, `reminder_sent_at`, `created_at`. Unique (user_id, entry_number). Explicit per-operation RLS (own rows only).

**on_this_day** (NEW v1.40 — daily bonus): `date` PK, `year`, `event`, `blurb`, `why_today`, `source_url`, `source_title`, `category`, `created_at`. RLS: authenticated select + insert. Facts from Wikipedia's on-this-day feed; Claude only frames. One shared card per date.

**RLS helpers/policies (v1.40):** `is_admin()` SECURITY DEFINER fn (avoids recursion in profiles policies); `feedback_update_admin`; `profiles_update_admin` (lets admins edit any profile, e.g. add phone).

**Edge functions + pg_cron:** `send-daily-reminder` (0 12 * * *), `send-practice-reminder` (0 * * * *), `send-weekly-wrap` (0 11 * * *), `send-lockin-review` (0 14 * * *), **`generate-on-this-day` (5 0 * * *, NEW v1.40 — GETs the `/api/on-this-day` Next route, NOT an edge function)**. Cron-called edge functions must have **Verify JWT OFF**. Source in `supabase/functions/`.

**Server routes that need keys:** On This Day persistence needs **`SUPABASE_SERVICE_ROLE_KEY`** in Vercel (route self-caches via service role). Admin Analytics needs **`POSTHOG_PERSONAL_KEY`** (`phx_…`; the public `phc_` ingest key can't read the query API). Both are server-only env vars (no `NEXT_PUBLIC_`). NEVER ping the email senders for a "health check" — they actually send mail; use `/api/health` instead.
...
| `/profile` | User profile — Account tab (name/phone/email_reminders/About/Privacy links) + Badges tab |
| `/privacy` | Privacy policy page (added Jun 15) |
| `/admin` | Admin dashboard |
| `/login` | Email/password login + reset password |
| `/onboarding` | 6-screen beta commitment flow |
| `/review` | Spaced-repetition recall page — "Keep It Sharp" (added v1.39) |
| `/on-this-day` | On This Day archive (all daily bonus cards; admin backfill) — v1.40 |

**API routes (v1.40):** `/api/on-this-day` (Wikipedia+Claude, self-caches), `/api/health` (safe AI/env probe), `/api/admin/feedback-summary` (AI themes feedback), `/api/admin/analytics` (admin-authed PostHog query proxy).

### Design System (as of v1.37)

- **App background:** `#0e141c`
- **Card/surface:** `#1a2a3a`
- **Borders:** `rgba(255,255,255,0.07)` faint / `rgba(255,255,255,0.13)` highlighted
- **Typography:** DM Sans (UI) + DM Mono (labels/metadata)
- **Action strip:** dark pill, CHANGELOG · BUG · FEEDBACK · SIGN OUT (ABOUT removed Jun 15)
- **Library filter chips:** active = border + dim fill, inactive = transparent border
- **Bottom nav:** `#1a2a3a`, colored underline pip per active tab
- **Deep Cut FAB:** circle, bottom right, 82px above nav, cycles 7 category accent colors every 2.5s
...
**Current entry count:** 040 entries published. Next entry: 041 — CM (Communication). See State/onepercentstate.md for current rotation position.

*Entry manifest table removed from instructions — too large to maintain here. Source of truth is app-next/app/page.js ENTRIES array and app-next/public/entries/.*
...
CLAUDE_API_KEY=[set — Deep Cut, Lock It In, On This Day, feedback summary]; ANTHROPIC_API_KEY fallback
SUPABASE_SERVICE_ROLE_KEY=[set — On This Day route self-cache / cron persistence; server-only]
POSTHOG_PERSONAL_KEY=[NEEDED — phx_ personal key for admin Analytics query proxy; server-only]
```
